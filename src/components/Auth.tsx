...
export const Auth: React.FC = () => {
  const { user: authUser, userData, isAuthReady, loginAsDemoUser } = useAuth();
  ...
  useEffect(() => {
    const loadDemoProfiles = async () => {
      ...
      try {
        ...
        // Query users matching the active role tab
        const q = query(
          collection(db, 'users'), 
        ...
        setDemoProfiles(list);
      } catch (err) {
        console.error("Error loading demo profiles for tab:", activeTab, err);
      } finally {
    ...
    <div className={`flex flex-col items-center justify-center min-h-screen relative overflow-hidden px-4 font-sans transition-all duration-1000 bg-gradient-to-tr ${getContainerBg()}`}>
      ...
      >
        ...
        <div className="space-y-6">
          ...
          {/* Quick Sandbox Logins Divider */}
          <div className="relative flex py-1 items-center">
            <div className="flex-grow border-t border-slate-200"></div>
            <span className="flex-shrink mx-4 text-slate-400 text-[10px] font-bold uppercase tracking-wider">Or Sandbox Bypass</span>
            <div className="flex-grow border-t border-slate-200"></div>
          </div>

          <div className="space-y-3">
            <div className="bg-slate-50 border border-slate-200/65 p-4 rounded-2xl">
              <p className="text-slate-500 text-[11px] font-extrabold mb-3 uppercase tracking-wider flex items-center gap-1.5">
              ...
              {loadingProfiles ? (
                ...
                <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-1">
                  {demoProfiles.map((profile) => (
                    <button
                      ...
                      onClick={() => {
                        if (loginAsDemoUser) {
                          loginAsDemoUser(profile.uid, profile);
                        }
                      }}
                      className="flex items-center gap-3 p-2.5 bg-white hover:bg-slate-100/50 border border-slate-200/80 hover:border-slate-300 rounded-xl text-left transition-all group cursor-pointer active:scale-95"
                    >
                      <div className="w-7 h-7 rounded-lg bg-indigo-50 text-indigo-600 group-hover:bg-indigo-100 group-hover:text-indigo-700 flex items-center justify-center font-bold text-xs uppercase transition-colors shrink-0">
                        {profile.name ? profile.name.substring(0, 2) : 'D'}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[11px] font-bold text-slate-800 truncate group-hover:text-slate-950">{profile.name}</p>
                        <p className="text-[9px] text-slate-400 truncate mt-0.5">{profile.email || 'No email assigned'}</p>
                      </div>
                      <span className="text-[9px] font-bold text-slate-400 group-hover:text-indigo-600 bg-slate-50 group-hover:bg-indigo-50 px-2 py-1 rounded-md transition-colors uppercase">
                        Login
                      </span>
                    </button>
                  ))}
                ...
                <div className="space-y-2">
                  ...
                  {activeTab === 'admin' && (
                    <button
                      type="button"
                      onClick={() => loginAsDemoUser && loginAsDemoUser("is5B5zbkEbUNlwYGQGlXJiAt66A2", { name: "David Muchiri (Admin)", email: "davmuchiri48@gmail.com", role: "admin" })}
                      className="flex items-center justify-between w-full p-3 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl text-left transition-all cursor-pointer active:scale-95"
                    >
                      <div>
                        <p className="text-xs font-bold text-slate-800">David Muchiri (Admin)</p>
                        <p className="text-[10px] text-slate-400 text-slate-450 mt-0.5">davmuchiri48@gmail.com</p>
                      </div>
                      <span className="text-[9px] font-bold text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-md uppercase shrink-0">Load Admin</span>
                    </button>
                  )}
                  {activeTab === 'teacher' && (
                    <button
                      type="button"
                      onClick={() => loginAsDemoUser && loginAsDemoUser("tech_prof_jones", { name: "Prof. Alan Jones", email: "teacher1@school.com", role: "teacher" })}
                      className="flex items-center justify-between w-full p-3 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl text-left transition-all cursor-pointer active:scale-95"
                    >
                      <div>
                        <p className="text-xs font-bold text-slate-800">Prof. Alan Jones (Faculty)</p>
                        <p className="text-[10px] text-slate-400 text-slate-450 mt-0.5">teacher1@school.com</p>
                      </div>
                      <span className="text-[9px] font-bold text-purple-600 bg-purple-50 px-2.5 py-1 rounded-md uppercase shrink-0">Load Faculty</span>
                    </button>
                  )}
                  {activeTab === 'student' && (
                    <button
                      type="button"
                      onClick={() => loginAsDemoUser && loginAsDemoUser("stud_test_01", { name: "Alice Mwangi (Student)", email: "alice.mwangi@gmail.com", role: "student" })}
                      className="flex items-center justify-between w-full p-3 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl text-left transition-all cursor-pointer active:scale-95"
                    >
                      <div>
                        <p className="text-xs font-bold text-slate-800">Alice Mwangi (Student)</p>
                        <p className="text-[10px] text-slate-400 text-slate-450 mt-0.5">alice.mwangi@gmail.com</p>
                      </div>
                      <span className="text-[9px] font-bold text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-md uppercase shrink-0">Load Student</span>
                    </button>
                  )}
                </div>
              )}
              ...
              <div className="mt-3.5 pt-3 border-t border-dashed border-slate-200 text-[10px] text-slate-400 leading-normal font-medium flex flex-col gap-1.5">
                <span className="font-extrabold text-slate-500 uppercase tracking-wide">Cloned Workspace Guide:</span>
                <p>
                  To restore downloaded database packages for rebranding templates: Log in, click <b>Administration Settings</b> → <b>Database Restoration</b>, and drag or select your downloaded <code>.json</code> file to seed.
                </p>
              </div>
            </div>
          </div>
